...
class pred_model:
  def __init__(self):
    with tf.device('/GPU:0'):
      self.input_frames = tf.placeholder(tf.float32, shape=[None, None, 64, 64, 1], name='input_frames')#batch,sqe,x,y,features
      ...
      # optimizer
      self.optimizer = self.__adam_optimizer_op(self.fut_loss + self.weight_decay * self.__calc_weight_l2_panalty())
  ...
  with tf.Session(config=sess_config) as sess:
    ...
    for step in range(100000):
      ...
      if step % 40 == 0:
        ...
        output_vid = np.concatenate((np.squeeze((x_batch*255).astype(np.uint8))[0][0:opts.num_frames // 2], o_vid), axis=0)
